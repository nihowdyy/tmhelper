...
const imagesDir = path.join(__dirname, 'locations');
...
const extractPokemonName = (fileName) => {
    ...
    for (const prefix of prefixes) {
        if (parts[0] === prefix) {
            ...
            baseName = parts.slice(0, 2).join(' '); // Base name includes the prefix and one more word
      ...
      if (path.extname(file) === '.jpg') { // Adjust to your file type
        ...
        if(pokemonName) {
            if (!imageMap[pokemonName]) {
              ...
              imageMap[pokemonName].push(path.join('locations', file));
        }
      } else {
...
const imageMapPath = path.join(outputDir, 'imageMap.json');
